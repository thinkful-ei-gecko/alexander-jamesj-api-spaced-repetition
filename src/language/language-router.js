...
languageRouter.post('/guess', express.json(), async (req, res, next) => {
  ...
  try {
    ...
    const currentUserData = await LanguageService.getUsersLanguage(
      req.app.get('db'),
      req.user.id
    )
    ...
    if (result.isCorrect) {
      await LanguageService.updateLanguageWhenCorrect(
        req.app.get('db'),
        req.language.id,
        result
      )
    } else {
      await LanguageService.updateLanguageWhenIncorrect(
        req.app.get('db'),
        req.language.id,
        result
      )
    ...
    const userData = await LanguageService.getUsersLanguage(
      req.app.get('db'),
      req.user.id
    )
